...
export function createShopCreditClient(
  ...
  const _getShopCreditCached = cache(
    async (): Promise<CreditResponse | null> => {
      const api = await createServerApi();
      const res = await api.get<ApiResponse<CreditResponse>>(
        '/wallet/credit/me',
        {
          validateStatus: (s) =>
  ...
  const _getShopRevenueSummaryCached = cache(
    async (days: number): Promise<GetShopRevenueSummaryResponse> => {
      ...
      return {
        ...
        points: Array.isArray(res.data?.data?.points)
  ...
  function getShopCredit(): Promise<CreditResponse | null> {
    return _getShopCreditCached();
  }

  function getShopCreditTransactions(
  ...
  function getShopRevenueSummary(days = 7) {
    return _getShopRevenueSummaryCached(days);
  }

  async function createShopPayout(
  ...
  async function getShopPayoutById(payoutId: string) {
    const api = await createServerApi();
  ...
  async function updateShopPayoutStatus(
    payoutId: string,
  ...
  async function deleteShopPayout(
    payoutId: string,
  ): Promise<ShopPayoutResponse> {
    const api = await createServerApi();
