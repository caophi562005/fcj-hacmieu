...
type BrandOption = GetManyBrandsResponse['brands'][number];
type CategoryOption = GetManyCategoriesResponse['categories'][number];
...
export type ProductImageUploadInput = { base64DataUrls: string[] };

export type SkuImageUploadInput = {
  skuIndex: number;
  skuValue: string;
...
export type ProductFormLookups = {
  loadChildCategories: (parentCategoryId: string) => Promise<CategoryOption[]>;
  loadDistricts: (provinceId: number) => Promise<DistrictResponse[]>;
  loadWards: (districtId: number) => Promise<WardResponse[]>;
};
...
export function ProductForm({
  ...
  const { loadChildCategories, loadDistricts, loadWards } = lookups;
  ...
  const [weightGram, setWeightGram] = useState(initial.weightGram);
  ...
  const submit = () => {
    ...
    if (adminMode && mode === 'edit' && !shopId.trim()) {
    ...
    if (
      ...
      return setError('Cân nặng phải từ 1 đến 50.000 gram.');
    }
    ...
    startTransition(async () => {
      ...
      if (mode === 'create' && result.id) {
        ...
        if (adminMode && mode === 'edit' && status !== initial.status) {
    ...
    <div className="max-w-5xl mx-auto space-y-5">
      ...
      <Section title="Thông tin chung">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <Field label="Cân nặng (gram)" required>
            <input
              type="number"
              className="input"
              min={1}
