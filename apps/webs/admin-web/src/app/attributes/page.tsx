...
import { AttributeCreateForm } from './ui';
...
function buildHref(query: { page?: number; name?: string }) {
  ...
  return qs ? `/attributes?${qs}` : '/attributes';
}
...
export default async function AttributesPage({
  ...
  const data = await getManyAttributes({ page, limit: 10, name: name || undefined });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-ink-muted text-sm mt-1">Tổng cộng {data.totalItems} thuộc tính.</p>
      </div>

      <form action="/attributes" method="GET" className="card p-4 flex gap-3 items-center">
        <input name="name" defaultValue={name} placeholder="Tìm theo tên attribute" className="input max-w-md" />
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100 text-ink">
              {data.attributes.length === 0 && (
                <tr>
                  <td colSpan={3} className="py-10 text-center text-ink-muted">Không có attribute.</td>
                </tr>
              )}
              {data.attributes.map((a) => (
                <tr key={a.id} className="hover:bg-surface-alt transition-colors">
                  <td className="py-3 px-4">
                    <Link href={`/attributes/${a.id}`} className="text-primary hover:underline cursor-pointer">
                      {a.id.slice(0, 8)}…
                    </Link>
                  </td>
                  <td className="py-3 px-4">{a.name}</td>
                  <td className="py-3 px-4">{a.url}</td>
                </tr>
              ))}
            </tbody>
          </table>
