...
export async function GET(request: NextRequest) {
  ...
  const appUrl = SellerWebConfig.webUrl();

  const error = searchParams.get('error');
  if (error) {
    console.error('[seller-web OIDC] Cognito error:', error);
  ...
  if (!cookieNonce || !cookieState) {
    console.error('[seller-web OIDC] Missing nonce/state cookie — expired?');
  ...
  try {
    ...
    console.error('[seller-web OIDC] Token exchange failed:', err);
