...
DROP FUNCTION `fn_wallet_available_balance`;
DROP PROCEDURE `sp_adjust_wallet`;
DROP PROCEDURE `sp_create_payout_request`;
...
BEGIN
    ...
    SELECT COALESCE(MAX(`balance`), 0) INTO v_balance
      FROM `Wallet` WHERE `userId` = p_user_id;
    RETURN v_balance;
END;
...
BEGIN
    ...
    SELECT `id`, `balance` INTO v_wallet_id, v_balance
      FROM `Wallet` WHERE `userId` = p_user_id FOR UPDATE;
...
BEGIN
    ...
    SELECT `id`, `balance` INTO v_credit_id, v_balance
      FROM `Credit` WHERE `shopId` = p_shop_id FOR UPDATE;
    ...
    INSERT INTO `CreditTransaction` (
        ...
        CONCAT('Tao yeu cau rut tien #', UPPER(RIGHT(p_payout_id, 8))), NOW(3)
    );
    COMMIT;
END;
